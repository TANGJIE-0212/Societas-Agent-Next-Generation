...
|---|---:|---|---|---|
| GDPval-style professional deliverable quality | 50 | GDPval、PaperBench、SWE-Lancer、MLE-bench | source materials、最终 PPT/Word/Excel/PDF、专家 rubric、LLM judge、人工抽样评审 | domain correctness、completeness、ready-to-use、professional quality、style/tone fit |
| Satisfaction / revision loop | 20 | Societas memo、online product telemetry、Chatbot Arena/MT-Bench 的 preference 思路 | download/export、return-to-project、manual edit distance、regenerate、abandon、repeat correction、revision rounds | 用户是否接受、少改、少重来、同类错误是否减少 |
| Process trust: trajectory / milestone / workflow | 20 | WorkArena、TheAgentCompany、tau-bench、AppWorld、OS-Themis、WebLINX | workflow trace、tool call log、approval log、action/observation、milestone evidence、verifier report、final state snapshot | workflow 是否跑通、tool/policy/state 是否正确、milestone evidence 是否完整、授权确认是否可审计 |
| Project-pack consistency | 5 | GDPval 扩展、PaperBench 层级任务、Societas Mid/Heavy samples | project_id、同项目多文件、版本 diff、实体/术语抽取、style profile、cross-artifact references | 跨文件术语、事实、结构、风格、引用一致性 |
| Cost / reliability | 5 | AI Agents That Matter、tau-bench、AndroidWorld | token cost、latency、same-task repeated runs、holdout/regression results | cost-normalized success、pass@1 / consistency、regression penalty |
...
### 6.3 Project-pack consistency 只占 5 分

Project-pack consistency 很重要，但不是每个任务都有。对单文件任务，它不应该占太大权重；对多文件项目包任务，可以作为 conditional metric 单独展开。

```text
Project-pack task: 单独计算 5 分 consistency
Single-artifact task: 这 5 分可以并入 Deliverable Quality 或 Satisfaction / Revision
```

这种设计避免因为少数 project-pack 场景，把所有任务的权重都拉偏。
