...
5. 再讲其他产品如何做 eval：Edge / PM Studio / CN AI Browser 等产品参考
...
## 5. 再讲其他产品如何做 Eval：产品参考

这一节把 Edge Journeys、PM Studio 和 CN AI Browser 放在同一层级。Edge Journeys 的公开页面和 CN AI Browser 正文已经读取；PM Studio 需要读到正文后再补结论，当前只保留来源链接。

### 5.1 Edge Journeys Evaluation
...
Edge Journeys Evaluation 的关键价值是：它不是只看生成内容好不好，而是先把用户意图和真实行为轨迹对齐，再评系统有没有在正确时机生成正确的 Journey。

它有两层 eval：

| 层 | 评什么 | 对 Societas 的启发 |
|---|---|---|
| Layer 1: Generation Quality | 单主题 browsing session 是否应该生成 Journey；生成的 card 质量、CTA accuracy、groundedness；同时包含 POS / NEG 任务 | Societas 也需要判断“该不该生成/该不该打扰”，而不是只评生成质量 |
| Layer 2: Selection Quality | 多主题、多天、带大量噪声的用户历史里，是否找出真正重要的 2-3 个 topic，排除已完成、敏感、过期任务，并正确去重/排序 | Societas 的在线满意度也要从噪声中抽真实工作信号：哪些项目还在继续，哪些已放弃，哪些修改代表偏好 |

Journeys 的方法对 Societas 有三个直接 learning：

1. **Intent-grounded evaluation**：先明确用户当时到底想做什么，再评价系统有没有帮上忙。Societas 不能只看 artifact 是否生成，还要知道用户目标、项目阶段和后续行为。
2. **POS / NEG 都要评**：不是所有信号都应该触发记忆、推荐、生成或适配。对 Societas 来说，已完成项目、敏感内容、噪声任务、一次性查询，都不应该被错误地写入用户偏好或 workflow。
3. **Selection 比 generation 更难**：真实用户历史有大量噪声。Societas 需要评系统是否能从 download、return、edit、regenerate、abandon 里挑出真正可用于 adaptation 的信号。

所以 Edge Journeys 对应的是 Societas 评分里的两块：

| Societas 模块 | 复用 Journeys 的什么 |
|---|---|
| Satisfaction / revision loop | 用行为轨迹判断用户是否接受、返回、继续、放弃，而不是只依赖显式反馈 |
| Attribution / adaptation | 判断哪些信号该进入 universal fix、project memory、personal preference，哪些应该 suppress |

在 Societas 的叙事里，这一节的作用不是“照抄 Edge”，而是说明：**产品级 eval 要从真实 user journey 中读信号，先判断 intent 和 signal quality，再决定是否 adaptation。**
...
PM Studio Eval 的核心不是单纯“生成 deliverable”，而是评 PM Agent 在真实 PM 工作中的理解、产出、可用性和行为底线。PM 任务里有大量没有实体产物的工作：决策参谋、优先级判断、风险识别、沟通协调、路线图取舍、用户研究洞察等。因此它和 GDPval 的差异非常关键：GDPval 偏标准化实体交付物，PM Studio 评估还要看过程中的推理、假设标注、工具调用是否合理。

PM Studio 的交付物分成 A/B 两套：

| 交付物 | 类比 | 作用 | 对 Societas 的启发 |
|---|---|---|---|
| A · 抽象判据 | 大陆法系 / 成文法 | 21 个评分单元 PASS/FAIL rubric：行为底线 6、理解 7、产出 6、可用 2 | Societas 也需要一套通用 rubric，可以评新题、新领域、新用户场景 |
| B · Golden Case 库 | 海洋法系 / 判例法 | 约 50 条人工校验 case GT，每条是自包含 input + intent + key_elements | Societas 日常迭代更应该跑 case GT 回归，而不是每次重新请专家判断 |

PM Studio 的 21 维评分单元：

| 层 | 维度 |
|---|---|
| 行为底线 | BL1 不编造、BL2 不越权、BL3 标不确定、BL4 识别风险、BL5 工具纪律、A3 错误恢复 |
| L1 理解 | C1 信息抽取、C2 多源整合、C3 时序纠错、C4 噪音过滤、C5 提问质量、C6 洞察提炼、A1 多步规划 |
| L2 产出 | D1 意图对齐、D2 结构可读、D3 分析深度、D4 可执行性、D5 事实准确、D6 框架应用 |
| L3 可用 | U1 省力度、U2 自足度 |

PM Studio 的任务覆盖不是按 artifact 类型，而是按 PM 工作类型：

```text
需求定义、战略规划、路线图、决策参谋、竞品分析、用户研究、沟通协调、数据分析、发布运营
```

这点对 Societas 很重要：Societas 虽然核心产物是 PPT / Word / Excel，但用户真实工作也不总是“生成文件”。有些任务是在决定怎么写、要不要写、先写什么、风险在哪里、下一步怎么推进。

PM Studio 的 A/B 双轨方法：

```text
A 系统：rubric-based evaluation
  - 用抽象 21 维评没见过的新题
  - 用于泛化、发现新能力空白

B 系统：case GT-based regression
  - 每条 case 有真实 input + intent + key_elements
  - 日常 A/B 和回归主要跑 B
  - 不需要每次 PM 重新校准
```
...
| 路径 | 做什么 | 意义 |
|---|---|---|
| 自上而下 | 6 个行业框架 + PM 任务频次 -> 9 类 PM 任务 × 21 维考核矩阵 | 保证“该考”的能力不漏 |
| 自下而上 | 收集 PM 真实工作对话，与能力地图匹配 | 保证题目是真实发生的工作，不是想象出来的 |
| PM 校准 | 把通用维度翻译成每个 case 的 must / should / may include / must_not_include | 把 rubric 变成可复用的标准答案锚 |

PM Studio 和 GDPval 的差异：

| 维度 | GDPval | PM Studio |
|---|---|---|
| 评判对象 | 最终实体产物，例如文档、幻灯片、图、表 | 既评产出，也评理解、判断、推理、风险识别、工具纪律 |
| 任务类型 | 偏标准化职业交付物 | PM 真实工作对话，很多没有实体 deliverable |
| 人类基准 | 需要人类专家做同等任务作为对比 | 目标不是证明 AI 比人强，而是衡量 PM Agent 自身能力水平 |
| 评分机制 | rubric / deliverable preference | A 抽象 rubric + B case GT 回归锚 |

对 Societas 的直接 learning：

1. **不要只评 artifact**：专业文件生产前后有很多无形 PM-like 工作：澄清、取舍、风险、结构、下一步计划。
2. **A/B 双轨很适合 Societas**：A 是通用 rubric，B 是从 Mid+Heavy 用户真实项目沉淀的 Golden Case。
3. **日常迭代主要跑 B 系统**：一旦 case GT 题库稳定，prompt / workflow / model 改动可以自动回归，不必每次找专家。
4. **rubric 不等于 case GT**：rubric 是考纲，key_elements / assertions 才是评卷标准。
5. **真实对话要拆成单轮 checkpoint**：复杂多轮工作需要拆成自包含 input + intent + key_elements，避免跨题依赖。

因此 PM Studio 对应 Societas 评分里的三块：

| Societas 模块 | 复用 PM Studio 的什么 |
|---|---|
| GDPval-style professional deliverable quality | 从通用 rubric 到 case-level key_elements 的翻译方法 |
| Satisfaction / revision loop | 省力度、自足度、修改负担、PM 契合度验证 |
| Process trust: trajectory / milestone / workflow | 行为底线、多步规划、工具纪律、错误恢复、风险识别 |

### 5.3 CN AI Browser Eval Flow
...
CN AI Browser Eval Flow 的核心是：**产品回答和 caseGT 分两条线生成，最后才合并评估。** 产品在真实环境里回答；caseGT 则由冻结页面证据、clean 正文、prompt 和 rubric 生成。这样可以避免先看产品表现再改标准。

完整 flow：

```text
URL pool -> URL 固化和分类
prompt 设计 -> rubric 维度冻结
页面快照 -> 自动 clean 正文
分叉 A：收集真实产品回答
分叉 B：生成并锁定 caseGT
caseGT + 各产品回答 -> eval -> backlog
```

关键设计：

| 环节 | 做什么 | 对 Societas 的启发 |
|---|---|---|
| URL pool 固化 | 记录 URL 来源、筛选理由、网页类型、适用场景、case 版本和负责人 | Societas 的任务样本也要有来源、筛选理由、项目类型和版本，而不是临时挑样本 |
| Prompt 设计 | 按网页类型和用户意图设计 prompt，不只用“总结页面” | Societas 的 eval prompt 要按任务类型区分：课程、法律、公司文档、技术规格、研究方案 |
| Rubric 冻结 | 先冻结准确性/事实性、完整性、清晰度、互动性、Grounding；Safety 单独处理 | Societas 也应先冻结 rubric，再生成/评分，Safety 和 hard gate 不应被总分平均掩盖 |
| 页面证据保存 | 保存正文、HTML、截图、metadata、final URL、snapshot time、失败状态 | Societas 要保存 source material、artifact plan、生成参数、文件 hash、verifier result，保证可复查 |
| Clean 正文 | 自动抽取 clean 正文候选，并标注 warning / fallback / 噪音样本 | Societas 可类比为 source material clean-up：输入素材、网页资料、已有文档需要可复核的 clean context |
| 分叉 A 产品回答 | 在真实产品环境收集原始回答、成功/失败状态、quota、timeout、无法读取页面等 | Societas 评估要保留真实产品输出和运行状态，不要只保留最终文件 |
| 分叉 B caseGT | 用强模型基于 clean 正文、prompt、rubric 生成 candidate assertions，再由人或 QA agent review | Societas 可用 assertions 评 deliverable：关键事实、结构、引用、格式、风险点是否满足 |
| 合并评估 | caseGT + 各产品回答，用同一 judge 逐条 assertion pass/fail + reason | Societas 应用同一套 locked assertions 评不同版本/模型/工作流，输出 failure type 和 backlog |

caseGT 的关键不是“标准答案”，而是一组可判定 assertions：

```text
dimension: Accuracy / Completeness / Clarity / Interactivity / Grounding / Safety
level: critical / expected / aspirational
judgement: pass = 1 / 0 + reason
evidence_span: 对应网页原文片段
review_status: draft / reviewed / rejected / needs_fix
```

这对 Societas 很有价值，因为专业文件评估也不适合只有一个 gold answer。更合理的是把任务拆成 assertions：
...
哪些 claims 必须有来源
哪些行为是 critical fail
哪些只是 aspirational quality
```

公平性边界也很重要：CN AI Browser 不声称绝对公正，而是通过同一 URL、同一 prompt、同一 rubric、同一 locked caseGT、同一 judge、同一证据和同一输出格式来做到“尽量公平”。这正好适合 Societas：**我们不能保证 expert rubric 完美，但可以保证所有候选版本被同一套 evidence 和 assertions 评估。**

对 Societas 的直接 learning：

1. **先冻结 eval，再跑产品对比**：先定 source/context、prompt、rubric、caseGT，再评不同 agent / workflow。
2. **产品输出和 GT 生成分线**：真实产品回答是一条线，caseGT/assertion 生成是另一条线，最后才合并。
3. **用 assertions 替代单一标准答案**：适合评专业文档、法律/医疗/财务等复杂 deliverables。
4. **把 eval 输出转 backlog**：不要只看总分，要记录 failure type、critical fail 和可修复问题。
5. **Safety 单独做 gate**：不被总分平均掉。

CN AI Browser 对应 Societas 评分里的两块：

| Societas 模块 | 复用 CN AI Browser 的什么 |
|---|---|
| Process trust: trajectory / milestone / workflow | URL/prompt/rubric 固化、页面证据、产品回答 vs caseGT 双轨、统一 judge |
| GDPval-style professional deliverable quality | 用 assertion-level caseGT 评复杂输出，而不是只看一个总分或一句 judge |

### 5.4 三者合起来的 learning

三者不是重复的，它们分别覆盖 eval 的不同阶段：

```text
Edge Journeys:
从用户行为中识别 intent 和有效信号

PM Studio:
把真实工作能力拆成 rubric + Golden Case

CN AI Browser:
把产品输出和 GT 生成分线，最后用 locked assertions 统一评估
```

所以对 Societas 来说，比较完整的 eval pipeline 应该是：

```text
1. 从真实用户行为中识别 intent / signal
  借鉴 Edge Journeys

2. 把真实工作拆成 rubric + Golden Case
  借鉴 PM Studio
...
关于 GDPval 已经有 Golden Case 这件事：Societas 不需要从零发明 Golden Case 方法论，但仍然需要自己的 Societas-specific Golden Cases。

原因是：

| 问题 | GDPval 已有能力 | Societas 还需要补什么 |
|---|---|---|
| 职业交付物质量 | GDPval 已有真实职业任务、专家 rubric、专家交付物和 gold set | 复用其任务设计和专家评审方法 |
| Office artifact | GDPval 覆盖 documents / slides / spreadsheets 等交付物 | Societas 要补 Office verifier、版本 diff、局部 revision、安全打开/编辑等工程验收 |
| 真实核心用户 | GDPval 覆盖 44 个职业，但不等于 Societas 的 Mid/Heavy 用户分布 | Societas 要覆盖 JP OT、KE 初创顾问、CH 法律案件、BR 教育、KR 财务等真实样本 |
| 项目连续性 | GDPval 当前偏 one-shot deliverable | Societas 要补 project-pack、多文件一致性、同项目多轮修订 |
| 用户满意和少改 | GDPval 主要评 expert deliverable quality | Societas 要补 download/export、return-to-project、manual edit distance、repeat correction、abandon 等在线信号 |
| 过程可复查 | GDPval 不主要评 trajectory / workflow | Societas 要补 trace、milestone、caseGT assertions、verifier report、approval gate |

因此最合理的做法是：

```text
复用 GDPval 的 Golden Case 思想和部分方法
+ 用 PM Studio 的 A/B 双轨组织 Societas rubric 和 caseGT
+ 用 CN AI Browser 的 locked assertions 保证评估公平和可复查
+ 用 Societas Mid/Heavy 用户样本沉淀自己的 Golden Case
```

也就是说：**GDPval 是 north-star 和方法来源；Societas Golden Case 是产品落地所需的本地化题库。**
