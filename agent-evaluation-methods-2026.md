...
## 0. 推荐呈现逻辑：先讲清评测形式，再讲 Societas 的 Eval

这份报告先用一节交代“现在大家是怎么评 agent 的”——基本评测形式加上竞品做法，作为共同语言；主线仍然从 Societas 自己的产品问题讲起。行业 benchmark 是证据和方法库，不是故事起点。

推荐讲法：

```text
1. 先讲 Agent Eval 的几种形式：白盒（trace / 工具调用）、黑盒（评产物 / 答案）、单轮、多轮（含 AI 模仿用户），以及 online/offline、评分器等分类轴
2. 再讲竞品做法：Edge / PM Studio / CN AI Browser / Copilot 怎么落地这些形式
3. 再讲 Signals to Satisfaction memo：Societas 遇到了什么问题
4. 再讲解决问题的两步：Stage 1 Evaluation -> Stage 2 Adaptation
5. 再讲 Mid+Heavy 用户：我们的核心用户到底在用 Societas 做什么
6. 再讲行业 Agent Eval：只讲和 Societas 相关的几类 benchmark
7. 再讲闭环怎么做：信号采集 -> 归因 -> 应用
8. 最后引出我们要做的 Societas Eval：一个专业文件生产系统的 workbench
```
...
## 1. 先讲 Agent Eval 的几种形式

在进入 Societas 自己的产品问题之前，先用一节把“现在大家是怎么评 agent 的”讲清楚，作为后面所有讨论的共同语言。更系统的评测基础常识可参考《Demystifying evals for AI agents》一文。本章先给一张分类轴总览，再展开和 Societas 最相关的几条轴（白盒/黑盒、单轮/多轮、online/offline、评分器）；竞品的具体落地做法放到第 2 章对照。

### 1.1 评测分类轴总览

业内没有单一标准，但把主流平台（Evidently、LangSmith）和实操经验综合起来，agent / LLM eval 常用的划分轴有 8 条。它们彼此正交：任何一个具体 eval 都可以在每条轴上各占一个位置。

| # | 分类轴 | 一端 | 另一端 | 一句话区分 | 代表 |
|---|---|---|---|---|---|
| 1 | 评估对象 | Model eval（裸模型能力） | Product eval（整个系统：prompt+RAG+工具+逻辑） | 评“模型”还是评“产品” | MMLU vs 客服 bot 准确率 |
| 2 | 何时 / 何地 | Offline（上线前、测试集） | Online（生产中、实时流量） | “考试” vs “上岗后绩效” | SWE-bench 跑分 vs 生产监控 |
| 3 | 有无参考答案 | Reference-based（有 ground truth / golden） | Reference-free（无标准答案，按 rubric 直接打分） | 对答案 vs 评质量 | exact match vs LLM-judge |
| 4 | 谁来评（评分器） | 确定性程序（parser / assertion / 单测） | 人工 | 中间还有 ML 模型、LLM-as-judge；按可信度/成本/可扩展性排成梯度 | regex 检查 vs 专家盲评 |
| 5 | 白盒 vs 黑盒 | 白盒（trace、工具调用、中间步骤） | 黑盒（产物 / 答案 / 终态） | 评过程 vs 评结果 | OS-Themis vs SWE-bench |
| 6 | 单轮 vs 多轮 | 单轮（一次性交付） | 多轮（迭代 / 澄清，含 AI 模仿用户） | 一问一答 vs 连续协作 | GDPval vs tau-bench |
| 7 | 评分形式 | Pointwise（绝对分 / rubric） | Pairwise（偏好 / 盲评对比） | 打分 vs 二选一 | rubric 评分 vs Chatbot Arena |
| 8 | 评估场景 / 目的 | 对比实验 / stress-test / red-team / 回归 / 监控 / guardrails | — | 为什么而评 | 选型 vs 防越权 |

这 8 条轴里，和 Societas 最相关、本章重点展开的是：轴 5（白盒/黑盒）、轴 6（单轮/多轮）、轴 2（online/offline）和轴 4（评分器）。轴 1（model→product）和轴 2（offline→online）其实就是本报告主线——从 offline capability eval 走向 online satisfaction eval、从 Stage 1 走向 Stage 2——的底层框架。

下面逐条展开。

### 1.2 白盒 vs 黑盒：评过程还是评产物

黑盒（black-box）只看最终产物或答案：文件、回复、最终状态对不对，不关心 agent 内部怎么走到这一步。

白盒（white-box）看执行过程本身：reasoning trace、工具调用序列、中间观察、retry / recovery、是否在高风险动作前确认。

| 维度 | 黑盒 Eval | 白盒 Eval |
|---|---|---|
| 评估对象 | 最终产物 / 答案 / 终态 | trace、工具调用、中间步骤、决策路径 |
| 典型问题 | 文件对不对？答案命中 gold 吗？ | 调用了正确工具吗？有没有幻觉调用？错误后能否恢复？ |
| 优点 | 直观、易和业务结果对齐、适合排行榜和回归 | 能区分“结果错”和“过程错”、能发现 reward hacking、能定位失败阶段 |
| 缺点 | 漏掉危险过程、误伤合理替代路径、对开放任务不够细 | 成本高、需要 instrumentation、不应要求唯一正确路径 |
| 代表 | SWE-bench（测试通过）、tau-bench（终态数据库） | OS-Themis（milestone + evidence chain）、WebLINX（action history） |

对 Societas 来说，两者都要：黑盒看 PPT / Word / Excel 产物是否专业可用；白盒看从用户目标到产物交付的 trajectory、工具调用和授权确认是否可审计。

### 1.3 单轮 vs 多轮：含 AI 模仿用户

单轮（single-turn）：一个固定输入 -> 一个输出，评一次性交付质量。GDPval 当前主要是单轮交付物评估。

多轮（multi-turn）：在多次交互中评 agent，要求它澄清需求、读上下文、迭代修改、处理工具返回、等待审批。真实 coworker agent 大多是多轮的。

多轮里有一个关键做法是 **AI 模仿用户（simulated user）**：用一个 LLM 扮演用户，按预设画像和目标和被测 agent 多轮对话，从而可规模化、可复现地造出多轮交互，而不用每条都请真人。tau-bench 就是典型：模拟用户 + 工具 / API + domain policy，最后比对终态数据库。

| 维度 | 单轮 | 多轮 | 多轮 + AI 模仿用户 |
|---|---|---|---|
| 交互 | 一问一答 | 多次澄清、迭代、审批 | LLM 扮演用户驱动多轮对话 |
| 评什么 | 一次性产物质量 | 上下文积累、修订、长程推进 | 同多轮，但可规模化、可复现 |
| 代表 | GDPval、MLE-bench | TheAgentCompany、WebLINX | tau-bench、模拟用户 harness |
| 风险 | 看不到迭代和澄清能力 | 真人成本高、难复现 | 模拟用户可能不像真人、需校准 |

对 Societas 来说，用户真实工作是“短时间内围绕一个项目连续生产一组文件”，本质是多轮：同项目多次修订、跨文件一致性、版本演进。所以 Societas 既要单轮交付物评估，也要多轮项目连续性评估，并可以用 AI 模仿用户来规模化造多轮修订场景。

### 1.4 Online vs Offline：上线前考试还是上线后绩效

这是业内最主流的顶层划分（LangSmith、Evidently 都把它当第一层）。区别在于评测发生在什么时候、用什么数据。

Offline（离线）：上线前或开发迭代中，在策展好的测试集上跑，常带 ground truth，用于版本对比、回归、stress-test、red-team。

Online（在线）：上线后在真实生产流量上评，没有标准答案，靠 reference-free 打分加抽样人工，用于监控、guardrails、A/B test、读用户行为信号。

| 维度 | Offline Eval | Online Eval |
|---|---|---|
| 何时 / 何地 | 上线前、开发迭代中 | 上线后、生产实时 |
| 数据 | 策展测试集，常带 ground truth | 真实用户流量 / traces，无标准答案 |
| 评分方式 | 多用 reference-based（对 golden） | 多用 reference-free（按 rubric）+ 抽样人工 |
| 典型用途 | benchmarking、回归、stress-test、red-team | 监控 / observability、guardrails、A/B test |
| 代表 | SWE-bench、tau-bench、GDPval 跑分 | 生产 LLM-judge 监控、用户 accept / edit / abandon 信号 |

对 Societas 来说，这条轴正是本报告主线：memo 说的“从 offline capability eval 扩展到 online satisfaction eval”、“Stage 1 Evaluation → Stage 2 Adaptation”，本质就是 offline → online 的递进。Edge Journeys 从用户浏览历史读 intent，就是典型的 online eval。Societas 两端都要：offline 用 Golden Case 保证交付物质量和防回归，online 用 download / return / edit / regenerate / abandon 等真实信号评满意度和修订负担。

一个常被引用的落地节奏（Hamel Husain）：L1 单测断言 → L2 人工 + 模型评 trace → L3 A/B test，成本递增、频率递减，本质是 offline → online 加上评分器从程序到人工的组合。

### 1.5 评分器：从确定性程序到人工

“谁来评”是另一条关键轴。按可信度、成本、可扩展性，评分器从前到后排成一个梯度：确定性程序 → ML 模型 → LLM-as-judge → 人工。原则是：能用前面的，就不要只靠后面的。

| 评分器 | 评什么 | 优点 | 局限 |
|---|---|---|---|
| 确定性程序（parser / assertion / 单测） | 客观、硬性的对错：结构、格式、事实一致性 | 确定、可复现、零成本、能精确定位失败 | 只能评可程序化判定的部分 |
| ML 模型 | 毒性、情感、PII 等专项分类 | 比规则灵活、比 LLM 便宜 | 需训练 / 维护，覆盖面窄 |
| LLM-as-judge | 主观质量：完整性、专业性、语气、是否答到点 | 可评开放输出、可扩展 | 有 position / verbosity / self-enhancement 偏差，需 rubric 和校准 |
| 人工 | 高风险、模糊、价值密集、建 gold label | 最可信、能定义“好” | 贵、慢、难规模化 |

其中最靠前的“确定性程序”由三件套组成，对 Societas 的 Office verifier 尤其关键：

- parser（解析器）：把非纯文本产物拆成可检查的结构。例如 pptx parser 读出页数 / 每页标题 / 图片引用 / 模板色板；docx parser 读出章节 / 目录 / 表格 / 页眉页脚；xlsx parser 读出 sheet / 列名 / 公式 / 引用范围；JSON / SQL parser 校验格式合法、字段齐全。
- assertion（断言）：对解析出的结构写一条条只有 pass / fail 的判定。例如“PPT 必须有标题页且 ≥ N 页、无空白占位符”“法律 Word 里金额 / 日期 / 附件编号必须与源一致”“输出里不能出现内部 UUID”。
- 单测（unit test）：把一批 assertion 按场景组织起来批量自动跑。跑得快又便宜，所以可以每次改 prompt / 代码就全跑一遍（对应上面 Hamel 的 L1）。

三者关系：parser 把产物拆开 → assertion 逐条判定 → 单测批量组织运行。它们覆盖客观、硬性的对错（结构、格式、事实一致性）；内容写得好不好、专不专业这种主观质量，才轮到 LLM-judge 和人工。一句话原则：能 parse 的不靠眼看，能断言的不靠 LLM judge，能跑测试的不靠口头说完成。

### 1.6 这几种形式怎么组合

上面这些轴不是互斥的，而是相互正交的组合。一个成熟的 agent eval 通常是：

```text
黑盒终态验收（产物 / 答案对不对）
+ 白盒过程审计（trace / 工具调用 / 授权）
+ 单轮交付物质量
+ 多轮项目连续性（含 AI 模仿用户造多轮场景）
+ offline 回归（测试集 + Golden Case）和 online 满意度（生产信号）两端都跑
+ 评分器分层：能 parse / assert 的先确定性程序，开放质量再上 LLM-judge / 人工
```

下面这三个竞品分别在这些形式上给了可借鉴的具体做法，第 2 章逐个展开：Edge Journeys 偏“从多轮用户行为里读 intent 和信号”，PM Studio 偏“把单轮 / 多轮工作拆成白盒 rubric + 黑盒 case GT”，CN AI Browser 偏“黑盒产物 + 白盒证据分线生成再统一评估”。

---

## 2. 再讲竞品做法：Edge / PM Studio / CN AI Browser / Copilot

这一章把几个竞品的 eval 做法放在一起对照，看它们如何落地第 1 章那几条轴。它们覆盖 eval 的不同阶段，最后收敛成 Societas 可复用的设计原则。

### 2.1 竞品参考一：Edge Journeys Evaluation

Sources:

- Journeys Evaluation Dataset: https://journeys.onemilelab.com/intent-grounded-eval/
- Journeys Evaluation / GDPval analysis notes: https://echoextension.feishu.cn/wiki/JLkQw6pwjiA2qrkCKaGcRaMznNb?from=from_copylink

它做什么：Edge Journeys 从海量用户的 Edge 标签页（tabs）和浏览行为里，识别用户下一步可能要做的 action，并做成一张可点击的 Action Card（Journey）。本质是"从真实浏览轨迹里读 intent，在正确时机推正确建议"。

它的 eval 怎么做：

- Golden case：一部分大模型生成，一部分从用户真实数据抽取（混合来源）。
- Rubric：自己设计——分两层，Layer 1 评生成质量（该不该生成 Journey、card 质量、CTA accuracy、groundedness，含 POS / NEG），Layer 2 评选择质量（从多天多主题噪声历史里挑出真正重要的 2-3 个 topic，排除已完成/敏感/过期，并去重排序）。
- 评测方式：LLM-as-judge，按 rubric 打分。

我们的 learning：

1. **先对齐 intent 再评**：不能只看 artifact 是否生成，要先知道用户当时想做什么、项目处在什么阶段。
2. **POS / NEG 都要评**：已完成、敏感、噪声、一次性任务不应被错误写入偏好或 workflow。
3. **Selection 比 generation 难**：Societas 也要从 download / return / edit / regenerate / abandon 里挑出真正可用于 adaptation 的信号，而不是把所有行为都当满意度。

### 2.2 竞品参考二：PM Studio Eval

Sources:

- `[V5] PM Context→Deliverable Eval 框架设计`: https://echoextension.feishu.cn/docx/IhRmd2uZ9o9UbmxsQuTcss0Wnac
- `LLM_AS_EXEC_Overview_v3_light.html`: https://microsoftapc-my.sharepoint.com/:u:/g/personal/zhangjingwei_microsoft_com/IQDjMSSKbl_FTbFShNdK8zbLAX7KWZf512YHnfcMPgeIfzQ?e=vrNtYr
- `v3-eval.zip`: https://microsoftapc-my.sharepoint.com/:u:/g/personal/zhangjingwei_microsoft_com/IQA0lYsjc_2OR6ec_sRbWaB3AfG3E5x2jVWFFHi9Uv9pA6A?e=klsBL2

它做什么：PM Studio 评一个 PM Agent 在真实产品经理工作里的表现——不只看产出的文档/deck，还看理解、判断、推理、风险识别和工具纪律。很多 PM 任务（决策参谋、优先级、沟通协调）根本没有实体产物。

它的 eval 怎么做：

- Golden case：题目两部分构成——(a) 大模型生成，(b) 从真实 PM 工作对话抽取，最后再人工校验。
- Rubric：A 套是专家自上而下设计的 21 维 PASS/FAIL 评分单元（行为底线 / 理解 / 产出 / 可用四层），再翻译成每条 case 的 must / should / may / must_not include。
- 评测方式：LLM-as-judge 按 rubric 和 key_elements 逐条判 PASS/FAIL，日常回归主要跑 B 套 case GT。

我们的 learning：

1. **不要只评 artifact**：专业文件生产前后有很多无形工作（澄清、取舍、风险、结构、下一步）。
2. **A/B 双轨适合 Societas**：A 是通用 rubric，B 是从 Mid+Heavy 用户真实项目沉淀的 Golden Case，日常迭代主要跑 B。

### 2.3 竞品参考三：CN AI Browser Eval Flow

Source:

- `CNAIBrowser_Eval_Flow_v2 1.html`: https://microsoftapc-my.sharepoint.com/:u:/g/personal/zhangjingwei_microsoft_com/IQDzlxN-ECkRTJ6F0duPKmUeAe69MA9I4TeEkRb06K51VLI?e=to7JVg

它做什么：CN AI Browser 评 AI 浏览器在真实网页上的回答质量（总结、问答、信息抽取），并能在同一套标准下横向对比多个产品 / 模型。

它的 eval 怎么做：

- Golden case：从 Copilot 的一批 Top 网页里挑出来。
- Rubric：自己设计（Accuracy / Completeness / Clarity / Interactivity / Grounding，Safety 单独 gate）。
- 评测方式：用大模型（LLM-as-judge）来判分。

我们的 learning：

1. **先冻结再跑对比**：先把素材、prompt、rubric、标准答案定死，再评不同 agent / workflow，避免看了结果再改标准。
2. **用 assertions 替代单一标准答案**：开放任务没有唯一写法，按"该满足的关键点有没有满足"来判，更适合专业文档；Safety 单独做 gate，不被总分平均掉。

### 2.4 竞品参考四：Copilot

它做什么：Copilot 评助手在各类任务上的回答质量。

它的 eval 怎么做：

- Golden case：题目两部分构成——一部分人工抽取，一部分大模型设计。
- Rubric：自己设计，但不细化到每一条 case，而是按 case 的类别，每一类共用一套评分标准。
- 评测方式：用大模型（LLM-as-judge）来判分。

我们的 learning：rubric 不一定要做到每条 case——按任务类别共用一套标准更轻量，适合 case 多、类型相对收敛的场景。

### 2.5 总结：构建一个评测器需要什么

把这几个竞品抽象一下，会发现要构建一个评测器，本质上只需要两大类东西：**Golden Case**（考什么题）和 **Rubric**（怎么判分）。几家都是 LLM-as-judge，差别主要在这两样东西怎么来。

| 竞品 | Golden Case 怎么来 | Rubric 怎么来 |
|---|---|---|
| Edge Journeys | 大模型生成 + 真实用户数据抽取（混合） | 自己设计：两层（生成质量 + 选择质量），含 POS / NEG |
| PM Studio | (a) 大模型生成 + (b) 真实 PM 对话抽取，最后人工校验 | 自己设计：A 套专家自上而下的 21 维 PASS/FAIL，翻译成每条 case 的 key_elements |
| CN AI Browser | 从 Copilot 用户 browse 的 Top 网站上找 | 自己设计：Accuracy / Completeness / Clarity / Interactivity / Grounding（Safety 单独 gate），先冻结 |
| Copilot | 人工抽取 + 大模型设计（混合） | 自己设计：按 case 类别给评分标准，不细化到每条 case |

可以看出 Golden Case 的来源无非几种：**论文 / 学术 benchmark、专家设计、真实用户数据抽取、大模型合成**——多数产品是混合用。而 Rubric 都是自己设计，区别在粒度：有的细化到每条 case（PM Studio 的 key_elements），有的只到类别（Copilot）。

把这几个产品（含 Societas 自己，以及作为对照的 GDPval）按第 1 章的分类轴摆一摆，会更清楚它们的共性和差异：

| 产品 | 评估对象 | online/offline | 参考答案 | 评分器 | 白盒/黑盒 | 单轮/多轮 | 评分形式 |
|---|---|---|---|---|---|---|---|
| Edge Journeys | product | offline | 无标准答案 / rubric | LLM-as-judge | 白盒+黑盒 | 单轮 | 绝对分 |
| PM Studio | product | offline | 无标准答案 / rubric | LLM-as-judge | 白盒+黑盒 | 多轮 | 绝对分 |
| CN AI Browser | product | offline | 无标准答案 / rubric | LLM-as-judge | 黑盒 | 单轮 | 绝对分 |
| Copilot | product | offline | 无标准答案 / rubric | LLM-as-judge | 白盒+黑盒 | 多轮 | 绝对分 |
| Societas | product | offline | 无标准答案 / rubric | LLM-as-judge | 黑盒 | 单轮 | 绝对分 |
| GDPval（对照，正经 LLM eval） | — | offline | 有标准答案 / 偏好评分 | 专家盲评 | 黑盒 | 单轮 | 偏好（pairwise） |

共性很明显：这几个产品级 eval 几乎都是 **product + offline + 无标准答案 rubric + LLM-as-judge + 绝对分**；差异主要在两条轴——**白盒/黑盒**（要不要看过程）和**单轮/多轮**（要不要连续交互）。而正经面向 LLM 能力的 GDPval 走的是另一条路：有标准答案、偏好评分。

对 Societas 的结论：

- **Golden Case**：GDPval（专家 / 论文来源）做 north-star，提供方法和任务设计；但 Societas 仍要从 Mid/Heavy 用户真实项目抽取自己的本地化 Golden Case，因为只有它能回答"对我们这群用户好不好、少不少改"。学术 / 专家来源偏 offline capability，真实用户来源偏 online satisfaction，两者互补。
- **Rubric**：自己设计，但可借三家的组织方式——PM Studio 的"通用 rubric → case 级 key_elements"翻译法、CN AI Browser 的"先冻结、assertion 级、Safety 单独 gate"，再加上 Office 文件特有的工程验收维度。

---

## 3. 先讲问题：为什么 Societas 不能只采用行业趋势
...
### 3.1 问题不是离线 benchmark 不够多，而是产品闭环缺 Stage 1
...
## 4. 再讲解决办法：Stage 1 Evaluation -> Stage 2 Adaptation
...
### 4.1 务实路径：online 采集 + offline 分析

真正的 online eval（实时打分、实时干预）很重——要实时 LLM-judge、实时 guardrail、低延迟，成本高、工程量大。但 online eval 难的只是"实时"这一点，它的价值大头在**信号本身**，不在实时算分。所以可以把它拆成两步，把难的部分推迟：

```text
online：只做"采集"——把用户真实行为信号原样记录下来
offline：批量回看这些信号，转成质量/满意度判断，再产出改进信号喂回系统
```

这正是 Stage 1 的落地方式：先在线采、再离线评，先把闭环跑起来，等稳定后再考虑把高价值的判断升级成实时。offline 分析阶段可以直接复用第 2 章那套 Golden Case + Rubric + LLM-judge——沉淀下来的真实信号，本身就是 Societas 最好的本地化 Golden Case 来源。

接下来要把这条路想清楚，需要回答三个问题，它们正好把"采集"和"分析"接成一个闭环：

1. **收集哪些信号**：download / export、return-to-project、manual edit distance、regenerate、abandon、repeat correction、revision rounds 等——哪些信号真正代表满意/不满意，哪些只是噪声。
2. **如何区分 universal signal 和 user-specific signal**：同一类问题是不是在很多用户身上反复出现（universal failure，该改全局），还是只对某个用户/项目成立（user-specific mismatch，该进项目或个人记忆）。
3. **分析完如何 apply 回去、apply 回哪些部分**：universal 改全局 workflow / verifier / rubric，user-specific 改 project memory / personal memory，并回测修订负担是否真的下降。

这三步会在后续章节展开，目标是把"采集（online）"和"分析+应用（offline）"形成一个可度量、可回测的闭环。

---

## 5. 再讲核心用户：Mid+Heavy Users 证明 Societas 是专业文件生产系统
...
## 6. 再讲行业 Eval：只讲和 Societas 相关的几类

行业 eval 很多，但和 Societas 最相关的是五类。

### 6.1 GDPval-like Deliverable Eval
...
### 6.4 Trajectory / Milestone / Evidence-chain Eval
...
### 6.5 Cost / Reliability / Regression Eval
...
第 4.1 节提出了务实路径——online 采集、offline 分析。这一章把它拆成三步落地，目标是把"采集（online）"和"分析 + 应用（offline）"接成一个可度量、可回测的闭环：

```text
1. 收集哪些信号
2. 如何区分 universal signal 和 user-specific signal
3. 分析完如何 apply 回去、apply 回哪些部分
  -> 回测：修订负担是否真的下降（闭环成立的判据）
```

### 7.1 第一步：收集哪些信号

原则是先采**行为信号**（用户做了什么，客观、低成本、不打扰），再在 offline 把行为信号翻译成**质量 / 满意度判断**。不要一开始就让用户显式打分——显式反馈稀疏且有偏。

**先说一个 Societas 特有的难点：Office 产物下载后是黑盒。** 代码类产品（如 Copilot）的"留存"天然可观测——接受的代码还在编辑器里，diff 一目了然，改没改、改了多少都看得见。但 PPT / Word / Excel 一旦 download 到本地 PowerPoint / Word，用户在本地改了什么，系统**看不见**。所以 download 本身有歧义：可能是"满意，拿走用了"，也可能是"系统做得不够，下载回去自己大改"。破解办法不是只看 download 这一个动作，而是**看 download 之后的后续行为序列来消歧**。

我们把信号按"满意强度"分级来 track，而不是只标"好/坏"：

| 方向 | 强度 | 代表信号 |
|---|---|---|
| 满意 | 强满意 | 分享 / 发送 / 演示；把产物复用为下一篇的模板或上下文；下载后不再回来修改 |
| 满意 | 中满意 | download / export（较强的可用信号）；同项目继续用 Societas 做下一篇 |
| 满意 | 弱满意 | 少量 in-app 微调后接受；停留浏览但无大改 |
| 不满意 | 弱不满意 | 少量局部 regenerate；小范围手动调整 |
| 不满意 | 中不满意 | 多轮 revision、整段 regenerate、大幅 in-app 编辑；下载后很快回来继续改 |
| 不满意 | 强不满意 | repeat correction（反复改同一类问题，系统性缺陷）；abandon（生成后不导出、不返回、不分享）；换方向重做 |

几个要点：

- **download 是较强的满意信号**，但不是终点——要用后续行为消歧：下载后"不再回来 / 去分享 / 下一篇复用它"→ 偏强满意；下载后"很快回来 regenerate / 大改"→ 其实是中/强不满意。
- **能把改后版本拉回系统最理想**：如果产品支持重新上传或基于改后版本继续，就能 diff 出用户在本地改了多少——这是唯一能真正"看见"Office 本地修改、最接近代码 retention 的途径。
- **判断靠信号组合 + 上下文**，不是单个信号：比如"大量 edit + 不 export + 不 return" ≈ 强不满意；"少量 edit + export + 下一篇复用" ≈ 强满意。
- **剔除噪声**：一次性查询、误操作、敏感 / 已完成项目，不应被当成满意度或写入偏好（借鉴 Edge Journeys 的 POS / NEG 思路）。

### 7.2 第二步：区分 universal vs user-specific signal

同一个"不满意"信号，可能是**所有人都会遇到的通用缺陷**，也可能**只对某个用户/项目成立**。区分错了，adaptation 就会走偏——把个人偏好改成全局，或把通用 bug 只塞进个人记忆。

判定主要看**信号是否跨用户复现**：

| 类型 | 判定线索 | 例子 |
|---|---|---|
| universal failure | 同一类问题在很多用户、很多项目上反复出现 | PPT 总生成空白占位符、公式经常坏、引用断裂 |
| user-specific mismatch | 只对某个用户/某类项目成立，别人没这问题 | 某律师要求特定条款措辞、某老师偏好某种课件结构 |
| project-specific constraint | 绑定某个具体项目/案件的事实和约束 | 案件 I00068 的债权人名单、某公司的术语表 |

实操做法：把每条 offline 判定按 `(failure_type, user, project, task_family)` 聚合，**算跨用户命中率**——高命中率 → universal；集中在单一 user/project → user-specific / project-specific。拿不准的丢人工或更强 judge 复核（借鉴评分器分层）。

### 7.3 第三步：apply 回哪里 + 回测

归因决定 adaptation 的目标层，不同类型改不同地方：

| 信号类型 | apply 回哪里 | 怎么改 |
|---|---|---|
| universal failure | 全局 workflow / verifier / rubric / prompt | 修通用能力，加 Office verifier 检查项，补 Golden Case 防回归 |
| user-specific mismatch | personal memory / 个人偏好 | 记住这个用户的偏好，下次默认应用 |
| project-specific constraint | project memory | 绑定到该项目，跨文件保持术语/事实/风格一致 |
| tool / artifact failure | 工程修复 + verifier | 修 bug，并加确定性检查防再次发生 |

闭环成立的判据不是"改了"，而是"**改完之后修订负担真的下降**"。所以每次 apply 后要回测同类任务：后续 revision rounds、manual edit distance、repeat correction、time-to-acceptance 是否下降，旧任务有没有退化（regression）。降了，这条信号才算真正闭环；没降，回到第二步重新归因。

```text
online 采集行为信号
  -> offline 翻译成质量/满意度判断（复用 Golden Case + Rubric + LLM-judge）
  -> 归因 universal / user-specific / project-specific
  -> apply 到 全局能力 / 个人记忆 / 项目记忆
  -> 回测修订负担是否下降
  -> 没降则重新归因，降了则沉淀成新的 Golden Case
```

---

## 8. 最后引出我们要做的 Eval：复用现有 Benchmark + Societas 适配层

基于第 2 节的 4 条设计原则，Societas 不需要从零自研一套 benchmark。更合理的做法是：**复用现有 benchmark 和产品 eval 的成熟方法，把它们组合成一个 Societas-specific eval workbench。**

具体到"在已有基线（GDPval Case + DeepSeek/CN AI Browser Case 这种黑盒看结果的 eval）之上，Societas 还要加强什么"，可以收敛成两块：**加强 Golden Case** 和 **加强 Rubric**。

**1. 加强 Golden Case**：在 GDPval Case 和 DeepSeek/CN AI Browser Case 之上，叠加**用户在 Societas 真实使用沉淀下来的 case**。这些真实 case 回答的是"对我们这群用户好不好、少不少改"，是学术/专家 case 替代不了的（来源见第 2.5 节，沉淀机制见第 7 章）。

**2. 加强 Rubric**：从"只黑盒看最终产物"扩展到四个维度——

| 加强项 | 含义 | 对应模块（见 8.2 权重） |
|---|---|---|
| (a) 过程评测 | 不只看结果，还评过程关键节点：trajectory / milestone / workflow、工具调用、授权确认是否可审计 | Process trust（20）|
| (b) 跨文档一致性 | 对生成多个文档的用户，检测术语 / 事实 / 风格 / 引用是否跨文件一致 | Project-pack consistency（5）|
| (c) 成本与可靠性 | 评 cost / latency、多次运行稳定性、改动后旧任务是否退化 | Cost / reliability（5）|
| (d) 满意度信号 | 把第 7 章那套用户行为信号（满意/不满意分级）纳入 rubric，用来评"用户是否少改、少重来"并驱动 agent 改进 | Satisfaction / revision loop（20）|

也就是说：**基线是 GDPval 式黑盒交付物质量（50 分仍是大头），Societas 的加强是把 Golden Case 本地化、把 Rubric 从黑盒结果扩到过程 + 一致性 + 成本 + 满意度。** 下面的 benchmark 选择和权重表就是这个思路的展开。
...
| Process trust: trajectory / milestone / workflow | 20 | [WorkArena](https://arxiv.org/abs/2403.07718)、[TheAgentCompany](https://arxiv.org/abs/2412.14161)、[tau-bench](https://arxiv.org/abs/2406.12045)、[AppWorld](https://arxiv.org/abs/2407.18901)、[OS-Themis](https://arxiv.org/abs/2603.19191)、[WebLINX](https://arxiv.org/abs/2402.05930)、CN AI Browser Eval Flow | workflow trace、tool call log、approval log、action/observation、milestone evidence、verifier report、final state snapshot、locked assertions | workflow 是否跑通、tool/policy/state 是否正确、milestone evidence 是否完整、授权确认是否可审计 |
...
### 9.2 为什么 Agent Eval 和普通 LLM Eval 不一样
...
### 9.3 当前 Agent Evaluation 的主流方法谱系

#### 9.3.1 Outcome / Task Success Evaluation
...
#### 9.3.2 State-based / Environment-backed Verification
...
#### 9.3.3 Programmatic Tests / Deterministic Verifier
...
#### 9.3.5 Milestone-level / Evidence-chain Evaluation
...
#### 9.3.6 LLM-as-a-Judge / Rubric Evaluation
...
#### 9.3.7 Human Review / Human-like Oversight
...
#### 9.3.8 Reliability / Repeated-run Evaluation
...
#### 9.3.9 Cost / Latency / Operational Evaluation
...
#### 9.3.10 Safety / Policy / Permission Evaluation
...
#### 9.4.3 GDPval：经济价值知识工作的 Deliverable Eval
...
#### 9.4.4 与 GDPval 相似的 Eval 方法：三层地图
...
### 9.8 Societas 场景下的 Eval-first 路线建议

#### 9.8.1 Societas 1.0：文档生成器 Eval
...
#### 9.8.2 Societas Coworker / Workflow Eval
...
#### 9.8.3 Societas 3.0：Self-evolution Eval
...
#### 9.8.4 综合结论：Societas Eval 应该像 GDPval + WorkArena + tau-bench + Office Verifier
...
#### 9.8.5 基于 Mid+Heavy User Samples 的 Societas-GDPval 任务设计
